...
# 强行将 Komari 绑在本地 3001 端口，把 7860 让给咱们的主控制台
export KOMARI_LISTEN="127.0.0.1:3001" 

cd /app

# ==========================================
# 1. 动态生成全栈热更控制台 (Web Terminal)
# ==========================================
...
<head>
    ...
    <title>Nexus Terminal // KOMARI</title>
...
<body class="h-screen w-screen overflow-hidden crt flex items-center justify-center p-2 sm:p-6">
    <div class="w-full h-full max-w-6xl bg-[#0a0a0a] border border-zinc-800 rounded-xl shadow-2xl flex flex-col relative z-10 overflow-hidden">
        ...
        <div id="output" class="flex-1 p-5 overflow-auto text-sm font-mono text-zinc-300">
            <div class="text-emerald-400 text-xl font-bold mb-4 tracking-widest">N E X U S // K O M A R I</div>
            <div class="text-zinc-400 mb-6">Environment ready.<br><span class="text-sky-400">⚡ Komari backend runs on port 3001. Tunnel handles routing.</span></div>
        </div>
    ...
    <script>
        ...
        socket.on('source-code-html', (code) => { document.getElementById('editorCode').value = code; document.getElementById('editorOverlay').classList.remove('hidden'); });
        socket.on('source-code-js', (code) => { document.getElementById('editorCode').value = code; document.getElementById('editorOverlay').classList.remove('hidden'); });
        function previewCode() { socket.emit('preview-source-html', document.getElementById('editorCode').value); window.open('/preview', '_blank'); }
        function deployCode() {
            const mode = document.getElementById('editorMode').value;
            const code = document.getElementById('editorCode').value;
            if(confirm('执行热部署将覆盖源码！确定吗？')) {
        ...
        socket.on('deploy-success', (mode) => {
            ...
            else { alert('后端逻辑已热加载！'); closeEditor(); }
        });
    </script>
</body>
</html>
...
module.exports = {
    handleHttp: function(req, res) {
        if (req.url === '/') { res.writeHead(200, { 'Content-Type': 'text/html' }); res.end(fs.readFileSync(uiFilePath, 'utf8')); } 
        else if (req.url === '/preview') { res.writeHead(200, { 'Content-Type': 'text/html' }); res.end(previewHTML); } 
    ...
    handleSocket: function(io, socket) {
        ...
        socket.on('deploy-source-js', (code) => {
            ...
            socket.emit('system', '[Backend] Source updated. Instructing bootstrapper...');
...
const server = http.createServer((req, res) => { logic.handleHttp(req, res); });
const io = new Server(server, { cors: { origin: "*" } });
io.on('connection', (socket) => { logic.handleSocket(io, socket); });
...
(
    ...
    # 2. 使用 PM2 守护 Komari 进程
    if [ -x "/app/komari_bin" ]; then
