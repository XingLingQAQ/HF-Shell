...
if [ "${_NODE_MAJ:-0}" -lt 22 ]; then
  ...
  curl -fsSL "https://nodejs.org/dist/v22.14.0/node-v22.14.0-linux-x64.tar.xz" \
    | tar -xJ -C /usr/local --strip-components=1 2>/dev/null && hash -r && echo "[Info] Node now $(node -v)" || echo "[Warn] node22 install skipped"
fi
...
# ---------- Chromium + 字体（Uptime Kuma Browser Monitor）----------
ensure_chromium() {
  if command -v chromium >/dev/null 2>&1 && [ -x "$(command -v chromium)" ]; then
    echo "[startup] chromium ok: $(chromium --version 2>/dev/null | head -1 || echo ok)"
    return 0
  fi
  echo "[startup] Installing chromium + fonts (Browser Monitor)..."
  ...
  if command -v chromium >/dev/null 2>&1; then
    echo "[startup] chromium installed: $(chromium --version 2>/dev/null || echo ok)"
    return 0
  fi
  echo "[startup] WARN: chromium install failed — Browser Monitor will not work"
  return 1
}

# 启动时安装（失败不阻塞）
ensure_chromium || true
...
<body class="h-screen w-screen overflow-hidden crt flex items-center justify-center p-2 sm:p-6">
    <div class="w-full h-full max-w-6xl bg-[#0a0a0a] border border-zinc-800 rounded-xl shadow-2xl flex flex-col relative z-10 overflow-hidden">
        ...
        <div class="border-b border-zinc-800 bg-[#0a0a0a] p-3 flex flex-wrap gap-2 text-xs font-medium z-20">
            ...
            <button onclick="sendCommand('chromium')" class="btn-action bg-zinc-900 border border-zinc-700 px-4 py-2 rounded-lg text-teal-400">🌐 Chromium</button>
            <button onclick="sendCommand('install chromium')" class="btn-action bg-zinc-900 border border-zinc-700 px-4 py-2 rounded-lg text-cyan-400">📥 装浏览器</button>
        ...
        <div id="output" class="flex-1 p-5 overflow-auto text-sm font-mono text-zinc-300">
            ...
            <div class="text-zinc-400 mb-6">Type 'help' / fix mieru.<br><span class="text-yellow-400">⚠️ Mieru needs TS5.8 + standalone (not TS7).</span><br><span class="text-teal-400">🌐 chromium / install chromium → Browser Monitor</span></div>
        </div>
        ...
        socket.on('command', (cmd) => {
            ...
            if (cmd === 'update kuma') {
                ...
                finalCmd = `
                    ...
                    echo "done"
                `;
            } else if (cmd === 'chromium') {
                finalCmd = `
echo "=== Chromium (Uptime Kuma Browser Monitor) ==="
echo -n "binary: "; command -v chromium 2>/dev/null || echo 'NOT FOUND'
echo -n "version: "; chromium --version 2>/dev/null || echo n/a
echo ""
echo "Env / paths Kuma looks for (auto-detect):"
echo "  CHROMIUM_BIN=\${CHROMIUM_BIN:-not set (default: chromium)}"
echo "  PUPPETEER_EXECUTABLE_PATH=\${PUPPETEER_EXECUTABLE_PATH:-not set}"
echo ""
echo "Fonts:"
dpkg -l fonts-indic fonts-noto fonts-noto-cjk 2>/dev/null | grep ^ii | awk '{print \$2,\$3}' || echo "  (not installed)"
echo ""
echo "Browser Monitor 用法:"
echo "  1) Uptime Kuma → Add Monitor → Monitor Type: Real Browser"
echo "  2) Kuma 会自动发现 chromium（路径: \$(command -v chromium)）"
echo "  3) 或者在 Kuma Settings → Monitor → Browser Executable Path 手动填路径"
echo "  4) 若在 Docker 内，可能需要 --no-sandbox；Kuma 2.x 默认已加"
`;
            } else if (cmd === 'install chromium') {
                finalCmd = `
set -e
echo "Installing chromium + fonts for Browser Monitor..."
apt-get update -qq && apt-get install -y --no-install-recommends chromium fonts-indic fonts-noto fonts-noto-cjk
apt-get clean && rm -rf /var/lib/apt/lists/*
chromium --version && echo "INSTALL OK — Browser Monitor ready" || echo "FAILED"
`;
