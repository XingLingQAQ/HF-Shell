#!/bin/bash
# ============================================================
# Forgejo + Nexus 完整面板（主题请手动安装 theme-github-dark.css）
# 覆盖: https://github.com/XingLingQAQ/HF-Shell/blob/main/Forgejo/startup.sh
# 数据目录仍用 /data/gitea（与 Gitea 兼容，可直接迁移）
# ============================================================
set -u
...
GITEA_ROOT_URL="${GITEA_ROOT_URL:-${FORGEJO_ROOT_URL:-}}"
GITEA_DISABLE_REGISTRATION="${GITEA_DISABLE_REGISTRATION:-true}"

mkdir -p /app /tmp /opt/forgejo "$GITEA_WORK_DIR" "$GITEA_CUSTOM/conf" /app/.pm2
chmod 777 /tmp /app 2>/dev/null || true
cd /app

# 非 root 用户（Forgejo/Gitea 均禁止 root）
if ! id git >/dev/null 2>&1; then
  useradd -m -u 1000 -d /home/git -s /bin/bash git 2>/dev/null \
    || adduser --disabled-password --gecos "" --uid 1000 git 2>/dev/null || true
fi
if id git >/dev/null 2>&1; then
  chown -R git:git "$GITEA_WORK_DIR" /opt/forgejo /home/git 2>/dev/null || true
else
...
ensure_forgejo() {
  if [ -x /opt/forgejo/forgejo ]; then
    log "forgejo: $(/opt/forgejo/forgejo --version 2>/dev/null | head -1 || echo ok)"
    return 0
  fi
  log "Downloading Forgejo..."
  mkdir -p /opt/forgejo
  # 从 codeberg 拉最新 linux-amd64
  LATEST=$(curl -fsSL https://codeberg.org/api/v1/repos/forgejo/forgejo/releases/latest 2>/dev/null \
    | grep -oE '"tag_name"[[:space:]]*:[[:space:]]*"[^"]+"' | head -1 | cut -d'"' -f4 | sed 's/^v//')
  if [ -z "$LATEST" ]; then
    LATEST=$(curl -fsSL https://codeberg.org/api/v1/repos/forgejo/forgejo/releases?limit=1 2>/dev/null \
      | grep -oE '"tag_name"[[:space:]]*:[[:space:]]*"[^"]+"' | head -1 | cut -d'"' -f4 | sed 's/^v//')
  fi
  [ -z "$LATEST" ] && LATEST="11.0.0"
  # 常见文件名: forgejo-X.Y.Z-linux-amd64
  for URL in \
    "https://codeberg.org/forgejo/forgejo/releases/download/v${LATEST}/forgejo-${LATEST}-linux-amd64" \
    "https://codeberg.org/forgejo/forgejo/releases/download/v${LATEST}/forgejo-${LATEST}-linux-amd64.xz" \
    "https://code.forgejo.org/forgejo/forgejo/releases/download/v${LATEST}/forgejo-${LATEST}-linux-amd64"
  do
    log "try $URL"
    if curl -fL "$URL" -o /tmp/forgejo.bin 2>/dev/null; then
      # xz?
      if file /tmp/forgejo.bin 2>/dev/null | grep -qi xz; then
        xz -d -c /tmp/forgejo.bin > /tmp/forgejo.out && mv /tmp/forgejo.out /tmp/forgejo.bin
      fi
      chmod +x /tmp/forgejo.bin
      # 简单校验：可执行且非太小
      SIZE=$(wc -c < /tmp/forgejo.bin | tr -d ' ')
      if [ "${SIZE:-0}" -gt 10000000 ]; then
        mv /tmp/forgejo.bin /opt/forgejo/forgejo
        echo "$LATEST" > /opt/forgejo/version
        log "installed forgejo $LATEST"
        return 0
      fi
    fi
  done
  log "ERROR: forgejo download failed"
  return 1
}
...
ensure_config() {
  ...
  if [ -f "$conf" ]; then
    log "app.ini exists, ensure theme + RUN_USER"
    sed -i 's/^RUN_USER *=.*/RUN_USER = git/' "$conf" 2>/dev/null || true
...
EOF
  chown -R git:git "$GITEA_WORK_DIR" 2>/dev/null || true
}

ensure_forgejo || log "WARN forgejo download failed"
ensure_config
...
<head>
    ...
    <title>Nexus Terminal // FORGEJO</title>
...
<body class="h-screen w-screen overflow-hidden crt flex items-center justify-center p-2 sm:p-6">
    <div class="w-full h-full max-w-6xl bg-[#0a0a0a] border border-zinc-800 rounded-xl shadow-2xl flex flex-col relative z-10 overflow-hidden">
        ...
        <div class="border-b border-zinc-800 bg-[#0a0a0a] p-3 flex flex-wrap gap-2 text-xs font-medium z-20">
            ...
            <button onclick="sendCommand('restart forgejo')" class="btn-action bg-zinc-900 border border-zinc-700 px-4 py-2 rounded-lg text-orange-400">♻️ 重启 Forgejo</button>
            <button onclick="sendCommand('update forgejo')" class="btn-action bg-zinc-900 border border-zinc-700 px-4 py-2 rounded-lg text-blue-400">🔄 更新 Forgejo</button>
            ...
            <button onclick="sendCommand('forgejo log')" class="btn-action bg-zinc-900 border border-zinc-700 px-4 py-2 rounded-lg text-zinc-300">📋 Forgejo 日志</button>
...
module.exports = {
  handleHttp(req, res) {
    ...
    if (path === '/api/ping' || path === '/health' || path === '/healthz') {
      ...
      res.end(JSON.stringify({ status: 'Forgejo nexus terminal is alive!', ts: Date.now() }));
      return;
    }
    res.writeHead(404, { 'Content-Type': 'text/html; charset=utf-8' });
    res.end('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>404</title></head><body style="background:#050505;color:#10b981;font-family:monospace;display:flex;align-items:center;justify-content:center;height:100vh;margin:0"><div style="text-align:center"><h1>404</h1><a href="/" style="color:#3b82f6">[ Return ]</a></div></body></html>');
  },
  handleSocket(io, socket) {
    socket.removeAllListeners();
    socket.on('command', (cmd) => {
      if (cmd === 'help') {
        socket.emit('output', 'status | pm2 status | restart forgejo | update forgejo | update tunnel | forgejo log | clear\n');
        socket.emit('system', '[Process completed]');
        return;
      }
      let script = cmd;
      if (cmd === 'status' || cmd === 'restart gitea') {
        // alias
      }
      if (cmd === 'status') {
        script = `
echo "=== Forgejo / Tunnel ==="
echo -n "forgejo: "; /opt/forgejo/forgejo --version 2>/dev/null | head -1 || echo missing
echo -n "ver: "; cat /opt/forgejo/version 2>/dev/null || echo none
echo -n "WORK_DIR: "; echo "${process.env.GITEA_WORK_DIR || '/data/gitea'}"
echo -n "Forgejo :3000: "; curl -fsS -o /dev/null -m 3 -w "%{http_code}" http://127.0.0.1:3000/ 2>/dev/null || echo down; echo
echo -n "ROOT_URL: "; grep -E '^ROOT_URL' /data/gitea/custom/conf/app.ini 2>/dev/null || echo n/a
${PM2} status 2>/dev/null || true
`;
      } else if (cmd === 'restart forgejo' || cmd === 'restart gitea') {
        script = `${PM2} restart forgejo --update-env; sleep 2; ${PM2} status forgejo; echo done`;
      } else if (cmd === 'stop forgejo' || cmd === 'stop gitea') {
        script = `${PM2} stop forgejo; echo stopped`;
      } else if (cmd === 'update forgejo' || cmd === 'update gitea') {
        script = `
set -e
LATEST=$(curl -fsSL https://codeberg.org/api/v1/repos/forgejo/forgejo/releases/latest | grep -oE '"tag_name"[[:space:]]*:[[:space:]]*"[^"]+"' | head -1 | cut -d'"' -f4 | sed 's/^v//')
LOCAL=$(cat /opt/forgejo/version 2>/dev/null || echo none)
echo "Local=$LOCAL Remote=$LATEST"
[ -z "$LATEST" ] && exit 1
[ "$LATEST" = "$LOCAL" ] && { echo already; exit 0; }
curl -fL "https://codeberg.org/forgejo/forgejo/releases/download/v${LATEST}/forgejo-${LATEST}-linux-amd64" -o /tmp/fj.new
chmod +x /tmp/fj.new; mv /tmp/fj.new /opt/forgejo/forgejo
echo "$LATEST" > /opt/forgejo/version
chown git:git /opt/forgejo/forgejo 2>/dev/null || true
${PM2} restart forgejo --update-env
echo done
`;
...
echo done
`;
      } else if (cmd === 'forgejo log' || cmd === 'gitea log') {
        script = `${PM2} logs forgejo --lines 100 --nostream 2>/dev/null || tail -n 100 /tmp/forgejo.log 2>/dev/null || echo no log`;
      } else if (cmd === 'tunnel log') {
        script = `${PM2} logs tunnel --lines 80 --nostream 2>/dev/null || echo no log`;
...
(
  log "start forgejo + tunnel..."
  $PM2 delete forgejo 2>/dev/null || true
  $PM2 delete gitea 2>/dev/null || true
  $PM2 delete tunnel 2>/dev/null || true

  if [ -x /opt/forgejo/forgejo ]; then
    chown -R git:git "$GITEA_WORK_DIR" /opt/forgejo 2>/dev/null || true
    sed -i 's/^RUN_USER *=.*/RUN_USER = git/' "$GITEA_WORK_DIR/custom/conf/app.ini" 2>/dev/null || true
    cat > /opt/forgejo/run.sh << 'RUNSH'
#!/bin/bash
set -u
export GITEA_WORK_DIR="${GITEA_WORK_DIR:-/data/gitea}"
export GITEA_CUSTOM="${GITEA_CUSTOM:-/data/gitea/custom}"
export HOME=/home/git USER=git USERNAME=git
cd "$GITEA_WORK_DIR" || exit 1
if id git >/dev/null 2>&1 && [ "$(id -u)" = "0" ]; then
  if command -v runuser >/dev/null 2>&1; then
    exec runuser -u git -- env HOME=/home/git USER=git \
      GITEA_WORK_DIR="$GITEA_WORK_DIR" GITEA_CUSTOM="$GITEA_CUSTOM" \
      /opt/forgejo/forgejo web \
        --config "$GITEA_WORK_DIR/custom/conf/app.ini" \
        --work-path "$GITEA_WORK_DIR" \
        --custom-path "$GITEA_CUSTOM"
  else
    exec su -s /bin/bash git -c "export HOME=/home/git USER=git GITEA_WORK_DIR='$GITEA_WORK_DIR' GITEA_CUSTOM='$GITEA_CUSTOM'; cd '$GITEA_WORK_DIR' && exec /opt/forgejo/forgejo web --config '$GITEA_WORK_DIR/custom/conf/app.ini' --work-path '$GITEA_WORK_DIR' --custom-path '$GITEA_CUSTOM'"
  fi
else
  export GITEA_I_AM_BEING_UNSAFE_RUNNING_AS_ROOT=true
  exec /opt/forgejo/forgejo web \
    --config "$GITEA_WORK_DIR/custom/conf/app.ini" \
    --work-path "$GITEA_WORK_DIR" \
    --custom-path "$GITEA_CUSTOM"
fi
RUNSH
    chmod +x /opt/forgejo/run.sh
    $PM2 start /opt/forgejo/run.sh \
      --name forgejo \
      --interpreter bash \
      --max-restarts 50 \
      --restart-delay 5000 \
    ...
    log "ERROR: forgejo binary missing"
  fi
  ...
  if [ -n "$CF" ]; then
    if [ ! -x /tmp/sysd ]; then
      ...
      chmod +x /tmp/sysd
      echo "$(curl -s https://api.github.com/repos/cloudflare/cloudflared/releases/latest | grep '"tag_name":' | head -1 | cut -d'"' -f4 || echo unknown)" > /app/.tunnel_version
    fi
  ...
  sleep 3
  $PM2 list || true
  curl -fsS -o /dev/null -m 3 -w "[startup] forgejo http=%{http_code}\n" http://127.0.0.1:3000/ 2>/dev/null || echo "[startup] forgejo not up yet"
